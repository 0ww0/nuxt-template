...
// only the SHA-256 hash is persisted; the raw 6-digit code is emailed once.
//
...
export const mfaCodes = pgTable(
  ...
  // Index the FK: deleteByUserId (newest-only on send, burn on verify/disable)
  // and the user-delete cascade would otherwise sequentially scan this table.
  (t) => [index('mfa_codes_user_id_idx').on(t.userId)],
)
